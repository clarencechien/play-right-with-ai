/**
 * Shopping List App - Fixed Version
 * All bugs have been resolved with explanations
 */
...
class ShoppingListApp {
    ...
    // FIX 1: Added proper validation for empty strings after trim
    /**
     *
     * @param {*} item - item 參數
     */
    addItem(item) {
        // Validation
        if (!item.name || item.name.trim() === '') { // FIX: Check trimmed value
        ...
        const newItem = {
            // FIX 2: Using Date.now() + Math.random() to ensure unique IDs
            id: Date.now() + Math.random(), // FIX: Added Math.random() for uniqueness
            name: item.name.trim(),
        ...
        // FIX 3: Save to storage after adding item
        this.saveToStorage(); // FIX: Added missing save
        this.render();
     ...
     */
    deleteItem(itemId) {
        // FIX 4: Use strict equality for comparison
    ...
    updateQuantity(itemId, newQuantity) {
        ...
        // FIX 5: Prevent quantity from being 0 or negative
        if (item && newQuantity > 0) { // FIX: Added quantity validation
    ...
    // FIX 6: Prevent negative budget limits
    /**
     *
     * @param {*} limit - limit 參數
     */
    setBudgetLimit(limit) {
        this.budgetLimit = Math.max(0, limit); // FIX: Ensure non-negative value
        this.saveToStorage();
        this.render();
    }

    // FIX 7: Exclude purchased items from budget calculation
    /**
     * calculateTotal 方法
     */
    calculateTotal() {
        return this.items.reduce((total, item) => {
            // FIX: Only include unpurchased items in total
            if (!item.purchased) {
    ...
    // FIX 8: Budget warning auto-disappears after 5 seconds
    /**
    ...
    showBudgetWarning() {
        ...
        if (warning) {
            ...
            // FIX: Added setTimeout to remove warning
            setTimeout(() => {
    ...
    // FIX 9: Use consistent localStorage key
    /**
     * saveToStorage 方法
     */
    saveToStorage() {
        try {
            ...
            // FIX: Using correct key 'shoppingList'
    ...
    loadFromStorage() {
        try {
            // Using consistent key 'shoppingList'
    ...
    // FIX 10: Re-bind event listeners after render
    /**
     * render 方法
     */
    render() {
        ...
        // FIX: Re-bind event listeners after render
        this.setupEventListeners();
    }
...
/**
 * FIXES APPLIED:
 * 1. ✓ Added validation for empty string after trim in addItem()
 * 2. ✓ Using Date.now() + Math.random() to ensure unique IDs
 * 3. ✓ Added saveToStorage() after adding item
 * 4. ✓ Using strict equality (===) for comparison in deleteItem()
 * 5. ✓ Prevent quantity from being 0 or negative in updateQuantity()
 * 6. ✓ Prevent negative budget limits with Math.max() in setBudgetLimit()
 * 7. ✓ Exclude purchased items from budget calculation
 * 8. ✓ Added setTimeout to auto-hide budget warning after 5 seconds
 * 9. ✓ Using consistent localStorage key 'shoppingList'
 * 10. ✓ Re-binding event listeners after render()
 */